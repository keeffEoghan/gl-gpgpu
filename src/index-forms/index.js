 ...
 * @param {number} [indexes=1] How many indexes, if any; result to be multiplied
 *   externally if not given.
 *
 * @returns {number} The number of vertexes needed for the given `form` to cover
 *   all steps of `states`; over all `indexes` if given, to be multiplied
 *   externally if not given.
 *
 * @remarks [[include:src/index-forms/readme.md]]
 */
export const indexForms = (states = 1, form = 2, indexes = 1) =>
  Math.max(1, (states-form)+1)*form*indexes;
